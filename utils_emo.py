...
def get_textual_examples(data_dir):
	...
	return responses, train_labels

def sep_emo(responses):
    emo_list = ['*', '_', '**', '__', '***', '___', '~~', '>!', '<!']
    emo_index_dict = {0:0, 1:0, 2:1, 3:1, 4:2, 5:2, 6:3, 7:4, 8:4}
    a = responses
    resp = []
    emo = []
    for com, rep in a:
        ...
        com_list = []
        rep_list = []
        com_emo_base = list(np.zeros((len(set(emo_index_dict.values())), )))
        rep_emo_base = list(np.zeros((len(set(emo_index_dict.values())), )))
        for word in wordpunct_tokenize(com):
            if word in emo_list:
                emo_index = emo_list.index(word)
                com_emo_base[emo_index_dict[emo_index]]=1
            else:
                com_list.append(word)
        com_sent = ' '.join(com_list)
        com_emo = com_emo_base
        for word in wordpunct_tokenize(rep):
            if word in emo_list:
                emo_index = emo_list.index(word)
                rep_emo_base[emo_index_dict[emo_index]]=1
            else:
                rep_list.append(word)        
        rep_sent = ' '.join(rep_list)
        rep_emo = rep_emo_base

        resp.append([com_sent, rep_sent])
        emo.append([com_emo, rep_emo])
    return resp, emo